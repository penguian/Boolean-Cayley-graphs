...
The relevant definitions and key properties are repeated below.
See the two papers for the proofs and for more details and references on bent functions.

The function $\sigma_m : \Z_2^{2 m} \To \Z_2$:
\begin{itemize}
 \item 
...
The function $\tau_m : \Z_2^{2 m} \To \Z_2$:
\begin{itemize}
 \item 
For $i \in \Z_{2^{2m}},$ $\tau_m(i) = 1$ if and only if the number of digits equal to 1 or 2 in the base 4
...
As noted in the paper on Hada\-mard matrices \cite{Leo14Constructions},
...
For a binary function $f : \Z_2^{2 m} \To \Z_2$, with $f(0)=0$ we now consider the simple undirected Cayley graph $\Cay(f)$
where $V(\Cay(f)) = \Z_2^{2 m}$ and for $i,j \in \Z_2^{2 m}$, $(i,j) \in E(\Cay(f))$ if and only if $f(i+j)=1$.
...
The paper on bent functions \cite{Leo15Bent} notes that 
since $\tau_m(i) = 1$ if and only if $\gamma_m(i)$ is symmetric but not diagonal,
Thus the subgraph $\varDelta_m[1]$ is isomorphic to the Cayley graph $\Cay(\tau_m)$.

The paper on bent functions \cite{Leo15Bent} uses these isomorphisms and the characterization of $\Cay(\sigma_m)$ and $\Cay(\tau_m)$ 
